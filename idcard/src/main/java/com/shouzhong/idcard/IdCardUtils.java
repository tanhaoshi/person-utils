...
public class IdCardUtils {
    ...
    public static final boolean initDict(final Context context) {
        File dir = context.getExternalFilesDir("idcard" + File.separator + "zocr0.lib");
        ...
        // step2: 检测字典是否正确
        Log.i("IdCardUtils","dir.getAbsolutePath() = " + dir.getAbsolutePath());
    ...
    private static final boolean checkDict(final String path) {
        try {
            ...
            if(code < 0){
                bytes = path.getBytes("GB18030");
                code = EXOCREngine.nativeInit(bytes);
                Log.e("kalu", "checkDict(GB18030) ==> code = " + code);
            }
            if (code < 0) {
            ...
            if(code < 0){
                bytes = path.getBytes("UTF-16BE");
                code = EXOCREngine.nativeInit(bytes);
                Log.e("kalu", "checkDict(UTF-16BE) ==> code = " + code);
            }
            if(code < 0){
                bytes = path.getBytes("UTF-16LE");
                code = EXOCREngine.nativeInit(bytes);
                Log.e("kalu", "checkDict(UTF-16LE) ==> code = " + code);
            }
            if(code < 0){
                bytes = path.getBytes("UTF-32");
                code = EXOCREngine.nativeInit(bytes);
                Log.e("kalu", "checkDict(UTF-32) ==> code = " + code);
            }
            if(code < 0){
                bytes = path.getBytes("ISO-8859-1");
                code = EXOCREngine.nativeInit(bytes);
                Log.e("kalu", "checkDict(ISO-8859-1) ==> code = " + code);
            }
            if(code < 0){
                bytes = path.getBytes("US-ASCII");
                code = EXOCREngine.nativeInit(bytes);
                Log.e("kalu", "checkDict(US-ASCII) ==> code = " + code);
            }
            if(code < 0){
                bytes = path.getBytes("CESU-8");
                code = EXOCREngine.nativeInit(bytes);
                Log.e("kalu", "checkDict(CESU-8) ==> code = " + code);
            }
            return code >= 0;
        } catch (Exception e) {
